...
import { Payment, Patient, ServiceRecord, Professional, ProfessionalLiquidation, GenericTransaction } from '@repo/database';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const genericTxRows = genericTxs
      // generic transactions are not linked to professionals; exclude them when filtering
      .filter((tx: any) => !professionalFilter || professionalFilter === 'Todos')
      .map((tx: any) => ({
        _id: tx._id.toString(),
        date: tx.date,
        amount: tx.amount,
